...
export function RegisterForm({
  ...
  // cette étape pour être rejoué à la fin, sans quoi le visiteur qui avait
  // choisi un plan atterrit sur le tableau de bord et l'a oublié.
  ...
  return (
    <div className="space-y-6">
      {/* En-tête — contextualisé quand le visiteur arrive d'une page vendeur */}
      <div className="space-y-1.5">
        <h1 className="text-2xl font-black text-foreground">
          {invite ? "Crée ta page, toi aussi" : "Créez votre boutique"}
        </h1>
        <p className="text-sm text-muted-foreground">
          {invite ? (
            <>
              Tu viens de voir la page de{" "}
              <span className="font-semibold text-foreground">{invite.name}</span>.
              La tienne est en ligne en 2 minutes.
            </>
          ) : (
            <>Crée ta page, vends depuis ta bio. C&apos;est gratuit pour commencer.</>
          )}
        </p>
      </div>

      {/* Ce que « gratuit » veut dire, et ce qui existe au-dessus. */}
      <StarterOffer />
