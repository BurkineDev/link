...
const SHOP = { slug: "wax" };
const mockPrisma = {
  shopLink: { findFirst: jest.fn(async () => (_link ? { ..._link, shop: SHOP } : null)) },
  pageBlock: { findFirst: jest.fn(async () => (_block ? { ..._block, shop: SHOP } : null)) },
};
...
function get(id: string, userAgent = DESKTOP, extra: { method?: string; headers?: Record<string, string> } = {}) {
  return GET(
    new NextRequest(`http://localhost:3000/go/${id}`, {
...
describe("GET /go/[lien]", () => {
  ...
  test("iOS + Facebook : page-pont HTML sous CSP à nonce, retour vers la boutique", async () => {
    ...
    expect(html).toContain('href="/wax">Retour à la boutique');
  });

  test("URL avec emoji ou alphabet non latin : encodée, jamais de 500", async () => {
    _link = { url: "https://wa.me/22670000000?text=Salut 👋" };
    const desktop = await get(ID);
    expect(desktop.status).toBe(302);
    expect(desktop.headers.get("location")).toBe("https://wa.me/22670000000?text=Salut%20%F0%9F%91%8B");

    _link = { url: "https://www.tiktok.com/@名前" };
    const android = await get(ID, ANDROID);
    expect(android.status).toBe(302);
    expect(android.headers.get("location")).toMatch(/^intent:\/\/www\.tiktok\.com\/@%E5%90%8D%E5%89%8D#Intent;scheme=https;/);

    _link = { url: "https://ex.com/a\nb" };
    expect((await get(ID)).headers.get("location")).toBe("https://ex.com/ab");
  });

  test("robots, aperçus, HEAD et préchargements : servis mais jamais comptés", async () => {
    _link = { url: "https://www.tiktok.com/@amy.creator" };
    const googlebot = "Mozilla/5.0 (Linux; Android 6.0.1; Nexus 5X) (compatible; Googlebot/2.1; +http://www.google.com/bot.html)";
    expect((await get(ID, googlebot)).headers.get("location")).toBe("https://www.tiktok.com/@amy.creator");
    expect((await get(ID, "facebookexternalhit/1.1")).status).toBe(302);
    expect((await get(ID, DESKTOP, { method: "HEAD" })).status).toBe(302);
    expect((await get(ID, DESKTOP, { headers: { "sec-fetch-mode": "no-cors" } })).status).toBe(302);
    expect((await get(ID, DESKTOP, { headers: { "sec-purpose": "prefetch" } })).status).toBe(302);
    await flush();
    expect(tracked).toEqual([]);

    expect((await get(ID, DESKTOP, { headers: { "sec-fetch-mode": "navigate", "sec-fetch-dest": "document" } })).status).toBe(302);
    await flush();
    expect(tracked).toEqual([`link:${ID}`]);
  });
