...
const critical = (over: Record<string, unknown> = {}) => ({
  ...
  context: { orderId: "o1" },
  dedupeKey: "webhook.late_payment:o1",
  ...over,
});
...
describe("recordOpsEvent", () => {
  test("un critique crée la ligne et envoie l'e-mail au fondateur, avec une clé d'idempotence", async () => {
    ...
    expect(_emails[0]!.idempotencyKey).toMatch(/^ops-alert\/webhook\.late_payment:o1\//);
  ...
  test("base injoignable : rien n'est levé, l'e-mail part quand même avec une clé horaire", async () => {
    _dbDown = true;
    const res = await recordOpsEvent(critical(), { now: T0 });
    expect(res).toEqual({ id: null, persisted: false, occurrences: 1, notified: true });
    expect(_emails[0]!.idempotencyKey).toBe("ops-alert/webhook.late_payment:o1/2026-09-14T03/fondateur@bio-lien.test");
  });
  ...
  test("sans ADMIN_EMAILS ni webhook, la ligne est écrite et rien ne part", async () => {
    ...
    expect(_rows[0]!.notifiedAt).toBeNull();
  });
